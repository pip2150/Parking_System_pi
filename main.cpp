...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        /*img_num = rand() % 37 + 1;
          fn = "C:/Users/dhrco/Desktop/자동차/plates/";*/
    ...
    while (1) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            divArea[j] = image(area[j]);
            matchTemplate(divArea[j], templ, result, TM_CCOEFF_NORMED);
            minMaxLoc(result, NULL, &maxVal, NULL, &maxLoc);

            //rectangle(divArea[j], maxLoc,
                ...
                for (int i = 0; i < (int)PossiblePlates.size(); i++) {
                    ...
                    if (response == 1) {
                        ...
                        for (int j = plateNumbers.size() - 1; j >= 0; j--) {
                            ...
                            string winName = Utils:: to_string(winNo) + Utils::to_string(i);
